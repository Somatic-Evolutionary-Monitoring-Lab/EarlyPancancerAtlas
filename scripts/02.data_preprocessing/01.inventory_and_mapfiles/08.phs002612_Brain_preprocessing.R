...
# Date: 2026-03-31
# Description: Annotate phs002612 GBM project data
# Project annotation number: 8
...
analysis_name <- 'mapping_and_inventory_files/phs002612_Brain'
out_name <- 'outputs'
...
biospecimen_metadata <- read_csv("/home/kh723/rds/rds-early-cancer_ev2-LH0AvU65IRI/EarlyPancancerAtlas/inputs/study_metadata/phs002612_Brain_SRA_Run_metadata.csv")
...
map_file <- biospecimen_metadata %>%
  mutate(germline = ifelse(is_tumor == "No", TRUE, FALSE),
         ...
         sample_type_detailed = case_when(
           grepl(".Re", `Library Name`) ~ "recurrent_gmb",
           grepl("Normal", `Library Name`) ~ "normal",
           TRUE ~ "primary_gmb"
         )
  ) %>%
...
# write out map file
write_delim(map_file_clinical, paste0(outputs.folder, date, "_phs002612_Brain_metadata_epa_mapping.tsv"), delim = "\t")
