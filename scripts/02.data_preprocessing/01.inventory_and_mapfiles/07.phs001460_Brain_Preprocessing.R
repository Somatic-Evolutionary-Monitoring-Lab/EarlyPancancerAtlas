...
# Date: 2026-03-31
# Description: Annotate phs001460 GBM project data
# Project annotation number: 7
...
analysis_name <- 'mapping_and_inventory_files/phs001460_Brain'
out_name <- 'outputs'
...
biospecimen_metadata <- read_csv("/home/kh723/rds/rds-early-cancer_ev2-LH0AvU65IRI/EarlyPancancerAtlas/inputs/study_metadata/phs001460_GMB_SRA_metadata.csv")
...
fastq_seq_dat <- read_tsv("/home/kh723/rds/rds-early-cancer_ev2-LH0AvU65IRI/EarlyPancancerAtlas/inputs/study_metadata/phs002612_Brain_seq_platform_info.tsv")
...
map_file <- biospecimen_metadata %>%
  mutate(germline = ifelse(Is_Tumor == "No", TRUE, FALSE),
         ...
         sample_type_detailed = case_when(
           grepl("Progression", `Library Name`) ~ "progression",
           grepl("Primary", `Library Name`) ~ "primary_tumour",
           grepl("non-enhancing", histological_type) ~ "non_enhancing_tumour",
           grepl("enhancing tumor", histological_type) ~ "enhancing_tumour",
           TRUE ~ "normal"
         )
  ) %>%
  ...
  mutate(
    ...
    GL_counter = as.integer(dense_rank(ifelse(germline, `Library Name`, NA))),
    # assign independent counters for tumour samples
    S_counter = as.integer(dense_rank(ifelse(!germline, `Library Name`, NA)))
  ) %>%
  # add sample ids
...
map_file_clinical <- map_file %>%
  mutate(
    ...
    run_dir = "phs001460_Brain",
    sequencer = gsub("Illumina\\s*|\\s+", "", Instrument),
    ffpe = ifelse(sample_class == "ffpe", TRUE, FALSE),
    seq_id = `Library Name`,
    seq_type = "WES",
    date_sequenced = format(as.Date(sub(" UTC$", "", map_file$ReleaseDate)), "%d/%m/%Y"),
...
# write out map file
write_delim(map_file_clinical, paste0(outputs.folder, date, "_phs001460_Brain_metadata_epa_mapping.tsv"), delim = "\t")
...
#' information on flowcell and lane was not avaliable
